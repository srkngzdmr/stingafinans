...
<script>
    ...
    function drawEye(eyeX, eyeY, mouseX, mouseY) {
        ...
        // İris (göz beyazı içinde sınırlandırılmış - DÜZELTME!)
        const angle = Math.atan2(mouseY - eyeY, mouseX - eyeX);
        const maxIrisDistance = eyeRadius - irisRadius - 2;
        const irisX = eyeX + Math.cos(angle) * maxIrisDistance;
        const irisY = eyeY + Math.sin(angle) * maxIrisDistance;
        ...
        // Göz ışığı (iris'in hareketine göre güncellenen - DÜZELTME!)
        ctx.fillStyle = '#ffffff';
        const lightX = irisX - 5;
        const lightY = irisY - 5;
        ctx.beginPath();
